...
def SD2_instance_generator(config: MyConfig) -> Tuple[np.ndarray, np.ndarray, float]:
    """
...
def matrix_to_text(job_length: np.ndarray, op_pt: np.ndarray, op_per_mch: float)->List[str]:
    """
...
def text_to_matrix(text: List[str])->Tuple[np.ndarray, np.ndarray]:
    """
    ...
    :return:  the matrix form of the instance
            ...
            op_pt: the processing time matrix with shape [N, M],
                where op_pt[i,j] is the processing time of the ith operation
                on the jth machine or 0 if $O_i$ can not process on $M_j$
            note: the index of operations is flattened (2d -> 1d)
    """
...
def pack_data_from_config(data_source: str, test_data: List[str])->List[Tuple[Tuple[List[np.ndarray], List[np.ndarray]], str]]:
    """
...
def generate_data_to_files(seed: int, directory: str, config: MyConfig):
    """
...
class CaseGenerator:
    ...
    def get_case(self):     # idx: int, not used
        """
        ...
        # line0 = '{0}\t{1}\t{2}\n'.format(self.num_jobs, self.num_mas, self.num_options / self.num_opes)       # not used
        lines_doc = []
